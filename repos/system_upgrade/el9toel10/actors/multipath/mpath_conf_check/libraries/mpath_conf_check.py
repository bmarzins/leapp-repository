...
from leapp.libraries.el9to10 import mpathfiles
...
def _report_config_dir(config_dir):
    create_report([
        ...
        reporting.Summary(
            ...
            'The only valid configuration directory is "{}". Any '
            'configuration files in "{}" will be moved to "{}".'.format(
...
def _report_config_dir_conflict(config_dir):
    create_report([
        ...
        reporting.Summary(
            'The multipath configuration option "config_dir" is set to '
            '"{}". During the upgrade, configuration files from "{}" '
            'will be moved to "{}". However, "{}" already contains '
            '.conf files. These existing files would be added to the '
            'multipath configuration after the upgrade. Please remove '
            'or relocate the files in "{}" before upgrading.'.format(
                config_dir, config_dir, _DEFAULT_CONFIG_DIR,
...
def check_configs(facts):
    ...
    bindings_file, wwids_file, prkeys_file = mpathfiles.config_file_locations(facts.configs)
