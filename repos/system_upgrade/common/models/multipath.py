...
class MultipathInfo(Model):
    ...
    """
    The location of bindings_file, if it should be copied to the same
    location in the target userspace. None if it should be copied to a
    different location (handled by upgrade-specific models).
    """

    wwids_file = fields.Nullable(fields.String())
    """
    The location of wwids_file, if it should be copied to the same
    location in the target userspace. None if it should be copied to a
    different location (handled by upgrade-specific models).
    """
    ...
    """
    The location of prkeys_file, if it should be copied to the same
    location in the target userspace. None if it should be copied to a
    different location (handled by upgrade-specific models).
    """
